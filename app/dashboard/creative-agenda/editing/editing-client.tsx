...
import { CreativeAgendaCard, EditingStatus } from '@/types';
import { Badge } from '@/components/ui/badge';
import { Button } from '@/components/ui/button';
import { DataTable } from '@/components/ui/data-table';
import { ColumnDef } from '@tanstack/react-table';
import { RefreshCw, Building2, FileText, Lightbulb, Calendar, Edit2 } from 'lucide-react';
import { getCardsByDepartment } from '@/app/actions/creative-agenda';
import { toast } from 'sonner';
import { format } from 'date-fns';
import { cn } from '@/lib/utils';

const statusColumns: { status: EditingStatus; title: string; isHighlighted?: boolean }[] = [
...
export function EditingClient() {
      ...
      cell: ({ row }) => {
        ...
        return (
          <div>
            ...
            {card.briefing_id && card.creator_id && (
              <div className="text-xs text-muted-foreground mt-1">
                Creator task for briefing
              </div>
            )}
          </div>
        );
  ...
  const loadCards = async () => {
    try {
      const fetchedCards = await getCardsByDepartment('editing');
  ...
  const getCardsByStatus = (status: EditingStatus) => {
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold">Editing</h1>
          <p className="text-muted-foreground mt-1">
            Manage content production for approved briefings
          </p>
        </div>
      </div>
