-- Fix the sync_briefing_card_status function to handle null auth
...
-- Fix the update_creative_agenda_updated_at function
CREATE OR REPLACE FUNCTION update_creative_agenda_updated_at()
RETURNS TRIGGER AS $$
DECLARE
    v_user_id TEXT;
BEGIN
    -- Get the user ID, fallback to NULL if not available
